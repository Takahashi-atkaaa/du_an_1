    ...
    <div class="container py-5">
        ...
        <?php if (isset($booking) && $booking): ?>
            <div class="invoice-card">
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <h5>Thông tin tài khoản khách hàng</h5>
                        <?php 
                            $khachHang = null;
                            $nguoiDung = null;
                            if (!empty($booking['khach_hang_id'])) {
                                ...
                                if (!empty($khachHang['nguoi_dung_id'])) {
                                    $nguoiDungModel = new NguoiDung();
                                    $nguoiDung = $nguoiDungModel->findById($khachHang['nguoi_dung_id']);
                                }
                            }
                        ?>
                        <p class="mb-1"><strong>Họ tên:</strong> <?php echo htmlspecialchars($nguoiDung['ho_ten'] ?? $booking['ho_ten'] ?? ''); ?></p>
                        <p class="mb-1"><strong>Email:</strong> <?php echo htmlspecialchars($nguoiDung['email'] ?? $booking['email'] ?? ''); ?></p>
                        <p class="mb-1"><strong>Số điện thoại:</strong> <?php echo htmlspecialchars($nguoiDung['so_dien_thoai'] ?? $booking['so_dien_thoai'] ?? ''); ?></p>
                        <?php if (!empty($khachHang['dia_chi'])): ?>
                            <p class="mb-0"><strong>Địa chỉ:</strong> <?php echo htmlspecialchars($khachHang['dia_chi']); ?></p>
                        <?php endif; ?>
                    </div>
                    <div class="col-md-6">
                        <h5>Thông tin tour đang đặt</h5>
                        <?php 
                            $tour = null;
                        ...
                        ?>
                        <p class="mb-1"><strong>Tour:</strong> <?php echo htmlspecialchars($tour['ten_tour'] ?? $booking['ten_tour'] ?? ''); ?></p>
                        <p class="mb-1"><strong>Loại tour:</strong> <?php echo htmlspecialchars($tour['loai_tour'] ?? $booking['loai_tour'] ?? ''); ?></p>
                ...
                <div class="table-responsive mb-4">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                <td><?php echo htmlspecialchars($tour['ten_tour'] ?? $booking['ten_tour'] ?? ''); ?></td>
                                <td class="text-end"><?php echo $booking['so_nguoi'] ?? 0; ?> người</td>
                                <td class="text-end"><?php echo number_format((float)($tour['gia_co_ban'] ?? $booking['gia_co_ban'] ?? 0)); ?> VNĐ</td>
                                <td class="text-end"><strong><?php echo number_format((float)($tour['gia_co_ban'] ?? $booking['gia_co_ban'] ?? 0) * ($booking['so_nguoi'] ?? 0)); ?> VNĐ</strong></td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
                                <th colspan="3" class="text-end">Tổng cộng:</th>
                                <th class="text-end"><?php echo number_format((float)($tour['gia_co_ban'] ?? $booking['gia_co_ban'] ?? 0) * ($booking['so_nguoi'] ?? 0)); ?> VNĐ</th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
