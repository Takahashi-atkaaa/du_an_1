...
class ThongBao 
{
    // Lấy tất cả yêu cầu tour của 1 khách hàng (cả chờ xác nhận và đã xác nhận)
    public function getAllByKhachHang($nguoi_gui_id) {
        $sql = "SELECT * FROM thong_bao WHERE loai_thong_bao = 'KhachHang' AND nguoi_gui_id = ? ORDER BY created_at DESC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([(int)$nguoi_gui_id]);
        return $stmt->fetchAll();
    }
    // Lấy các yêu cầu tour chờ xác nhận của 1 khách hàng
    public function getYeuCauTourChoXacNhan($nguoi_gui_id) {
        $sql = "SELECT * FROM thong_bao WHERE loai_thong_bao = 'KhachHang' AND nguoi_gui_id = ? AND trang_thai = 'DaGui' ORDER BY created_at DESC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([(int)$nguoi_gui_id]);
        return $stmt->fetchAll();
    }
