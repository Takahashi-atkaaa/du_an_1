...
--
-- Làm sạch dữ liệu: Cập nhật các foreign key không tồn tại thành NULL
-- 1. Làm sạch booking_id
UPDATE `giao_dich_tai_chinh` gd
LEFT JOIN `booking` b ON gd.booking_id = b.booking_id
SET gd.booking_id = NULL
WHERE gd.booking_id IS NOT NULL AND b.booking_id IS NULL;

-- 2. Làm sạch khach_hang_id
UPDATE `giao_dich_tai_chinh` gd
LEFT JOIN `khach_hang` kh ON gd.khach_hang_id = kh.khach_hang_id
SET gd.khach_hang_id = NULL
WHERE gd.khach_hang_id IS NOT NULL AND kh.khach_hang_id IS NULL;

-- 3. Làm sạch nguoi_thuc_hien_id
UPDATE `giao_dich_tai_chinh` gd
LEFT JOIN `nguoi_dung` nd ON gd.nguoi_thuc_hien_id = nd.id
SET gd.nguoi_thuc_hien_id = NULL
WHERE gd.nguoi_thuc_hien_id IS NOT NULL AND nd.id IS NULL;

-- 4. Làm sạch tour_id
UPDATE `giao_dich_tai_chinh` gd
LEFT JOIN `tour` t ON gd.tour_id = t.tour_id
SET gd.tour_id = NULL
WHERE gd.tour_id IS NOT NULL AND t.tour_id IS NULL;

-- Xóa các constraint cũ nếu tồn tại (để tránh lỗi khi chạy lại)
-- Xóa fk_gd_booking
SET @constraint_exists = (
    ...
    AND CONSTRAINT_NAME = 'fk_gd_booking'
);
SET @sql = IF(@constraint_exists > 0,
    'ALTER TABLE `giao_dich_tai_chinh` DROP FOREIGN KEY `fk_gd_booking`',
    'SELECT 1'
);
...
SET @constraint_exists = (
    ...
    WHERE CONSTRAINT_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'giao_dich_tai_chinh' 
    AND CONSTRAINT_NAME = 'fk_gd_khach'
);
SET @sql = IF(@constraint_exists > 0,
    'ALTER TABLE `giao_dich_tai_chinh` DROP FOREIGN KEY `fk_gd_khach`',
    'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @constraint_exists = (
    ...
    WHERE CONSTRAINT_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'giao_dich_tai_chinh' 
    AND CONSTRAINT_NAME = 'fk_gd_nguoi'
);
SET @sql = IF(@constraint_exists > 0,
    'ALTER TABLE `giao_dich_tai_chinh` DROP FOREIGN KEY `fk_gd_nguoi`',
    'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
-- Xóa giao_dich_tai_chinh_ibfk_1
SET @constraint_exists = (
    SELECT COUNT(*) 
    FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS 
    WHERE CONSTRAINT_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'giao_dich_tai_chinh' 
    AND CONSTRAINT_NAME = 'giao_dich_tai_chinh_ibfk_1'
);
SET @sql = IF(@constraint_exists > 0,
    'ALTER TABLE `giao_dich_tai_chinh` DROP FOREIGN KEY `giao_dich_tai_chinh_ibfk_1`',
    'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
ALTER TABLE `giao_dich_tai_chinh`
  ...
  ADD CONSTRAINT `giao_dich_tai_chinh_ibfk_1` FOREIGN KEY (`tour_id`) REFERENCES `tour` (`tour_id`) ON DELETE SET NULL;
...
-- ============================================================
-- MODULE: Hệ Thống Lương, Hoa Hồng & Thưởng cho HDV
-- ============================================================

-- Thêm cột commission_percentage vào bảng nhan_su (nếu chưa có)
SET @dbname = DATABASE();
...
-- Tạo bảng hdv_salary - Bảng lương HDV
...
-- Tạo bảng hdv_bonus - Bảng thưởng HDV
...
-- Tạo VIEW cho thống kê lương HDV
