...
class AdminController {
            // Xác nhận yêu cầu tour
            public function xacNhanYeuCauTour() {
                require_once 'models/ThongBao.php';
                $id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
                if ($id > 0) {
                    $thongBaoModel = new ThongBao();
                    $thongBaoModel->update($id, [
                        'trang_thai' => 'DaGui'
                    ]);
                    $_SESSION['success'] = 'Đã xác nhận yêu cầu tour!';
                }
                header('Location: index.php?act=admin/yeuCauTour');
                exit();
            }
        // Xem danh sách yêu cầu tour của khách hàng
        public function yeuCauTour() {
            ...
            // Lấy các yêu cầu tour gửi từ khách hàng
            $list = $thongBaoModel->getByLoai('KhachHang');
            $yeuCauTourList = [];
            foreach ($list as $yc) {
                // Tách nội dung
                ...
                // Lấy thông tin người dùng từ noi_dung
                $noi_dung = $yc['noi_dung'] ?? '';
                $parts = [];
                foreach (explode("\n", $noi_dung) as $row) {
                    $kv = explode(": ", $row, 2);
                ...
                $yeuCauTourList[] = [
                    ...
                    'ho_ten' => $parts['Tên'] ?? 'Ẩn danh',
                    'email' => $parts['Email'] ?? '',
                    'so_dien_thoai' => $parts['Số điện thoại'] ?? '',
                    'dia_diem' => $parts['Địa điểm'] ?? '',
                    'thoi_gian' => $parts['Thời gian'] ?? '',
                    'so_nguoi' => $parts['Số người'] ?? '',
                    'yeu_cau_dac_biet' => $parts['Yêu cầu đặc biệt'] ?? '',
                    'ngay_gui' => $yc['ngay_gui'] ?? ''
                ];
            }
            require 'views/admin/yeu_cau_tour.php';
        }
