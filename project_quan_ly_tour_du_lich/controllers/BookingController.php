...
class BookingController {
    ...
    // Đặt tour online (không cần đăng nhập)
    public function createOnline() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $soNguoi = isset($_POST['so_nguoi']) ? (int)$_POST['so_nguoi'] : 1;
            $ngayKhoiHanh = $_POST['ngay_khoi_hanh'] ?? '';
            $lichKhoiHanhId = isset($_POST['lich_khoi_hanh_id']) ? (int)$_POST['lich_khoi_hanh_id'] : null;
            $ghiChu = trim($_POST['ghi_chu'] ?? '');

            // Validate
            if ($tourId <= 0 || empty($hoTen) || empty($email) || empty($soDienThoai)) {
                $_SESSION['error'] = 'Vui lòng điền đầy đủ thông tin bắt buộc.';
                header('Location: index.php?act=tour/bookOnline&tour_id=' . $tourId);
                exit();
            }
            ...
            // Tìm hoặc tạo khách hàng
            $khachHang = $this->khachHangModel->findByNguoiDungId($nguoiDung['id']);
            if (!$khachHang) {
            ...
            // Tạo booking
            $tongTien = (float)$tour['gia_co_ban'] * $soNguoi;
            $bookingData = [
                'tour_id' => $tourId,
                'khach_hang_id' => $khachHangId,
                'so_nguoi' => $soNguoi,
                'ngay_khoi_hanh' => !empty($ngayKhoiHanh) ? $ngayKhoiHanh : null,
                'lich_khoi_hanh_id' => $lichKhoiHanhId,
                'tong_tien' => $tongTien,
                'trang_thai' => 'ChoXacNhan',
                'ghi_chu' => !empty($ghiChu) ? $ghiChu : null
            ];

            $inserted = $this->bookingModel->insert($bookingData);
            if ($inserted) {
                $_SESSION['success'] = 'Đặt tour thành công! Chúng tôi sẽ liên hệ với bạn sớm nhất. Mã booking: #' . $this->bookingModel->conn->lastInsertId();
                header('Location: index.php?act=tour/bookOnline&tour_id=' . $tourId);
            } else {
                $_SESSION['error'] = 'Không thể đặt tour. Vui lòng thử lại.';
                header('Location: index.php?act=tour/bookOnline&tour_id=' . $tourId);
            }
            exit();
        }
